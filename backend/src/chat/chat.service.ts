...
export class ChatService {
  ...
  async getOneChat(
    ...
    try {
      return this.prisma.chat.findMany({
        where: {
          users: {
            some: { id: { equals: usrChatId } },
          },
        },
      });
    } catch {
      throw new UnauthorizedException(
        `Couldn't find message with id ${usrChatId}`,
      );
    }
  }
  ...
  async getUserChats(
    ...
    try {
      ...
      throw new UnauthorizedException(
        `Couldn't find message with id ${id}`,
      );
    }
  }
  ...
  async getChatId(chatId: number): Promise<Chat> {
    try {
      ...
      throw new UnauthorizedException(
        `Couldn't find chat with id ${chatId}`,
      );
    }
  }
  ...
  async getCommunChat(
    ...
    try {
      ...
      if (communChat) return communChat[0];
    } catch (err) {
      throw new UnauthorizedException(
        `Error getting commun chat between ${sender} and ${receiver} : `,
        err,
      );
    }
  }
  ...
  async createChat(
    ...
    try {
      const _chat = await this.getCommunChat(
        senId,
        recId,
      );
      if (_chat)
  ...
  async deleteAllChats() {
    ...
    else return `Couldn't delete chats!`;
  }
