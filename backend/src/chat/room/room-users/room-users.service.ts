...
export class RoomUsersService {
  ...
  async getAllRoomUsers() {
    try {
      ...
      console.log(
        `Couldn't find users in this room: ${err}`,
      );
    }
  }
  ...
  async isRoomAdmins(
    ...
    try {
      const admin =
        await this.prisma.roomUsers.findMany({
          where: {
            AND: [
              { roomId: roomId },
              { userId: userId },
  ...
  async isRoomMemberAllowed(
    ...
    try {
      ...
      if (roomUser.length !== 1) return false;
  ...
  async getOneContact(
    ...
    const chat: Chat =
      await this.chatService.getCommunChat(
        userId,
        recId,
      );
  ...
  async createRoomUsers(
    ...
    try {
      ...
      console.log(
        `Couldn't create room users table: ${err}`,
      );
    }
  }
  ...
  async deleteAllRoomUsers() {
    ...
    else return `Couldn't delete room users!`;
  }
