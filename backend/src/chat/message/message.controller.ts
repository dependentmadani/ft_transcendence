...
export class MessageController {
  ...
  async getChatMessages(
    @Param('chatId', ParseIntPipe) chatId,
  ): Promise<Message[]> {
  ...
  async createChatMessage(
    @Body('msgChatId', ParseIntPipe)
    msgChatId: number,
    @Body('MessageSenId', ParseIntPipe)
    MessageSenId: number,
    @Body('textContent') textContent: string,
    @Body('type') type: string,
  ): Promise<Message> {
    return this.messageService.createChatMessage(
      msgChatId,
      MessageSenId,
      textContent,
      type,
  ...
  async createRoomMessage(
    ...
    return this.messageService.createRoomMessage(
      msgRoomId,
      MessageSenId,
      textContent,
      type,
