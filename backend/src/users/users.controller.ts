...
export class UsersController {
  ...
  async unFriendFriend(
    ...
    return await this.userService.unfriend(
      user.id,
      friendId,
    );
  }
  ...
  async blockFriend(
    ...
    const blockedFriend =
      await this.userService.blockFriend(
        user.id,
        friendId,
      );
  ...
  async unblockFriend(
    ...
    const unblock =
      await this.userService.unblockFriend(
        user.id,
        friendId,
      );
  ...
  async mutualFriends(
    ...
    const mutual =
      await this.userService.mutualFriends(
        user.id,
        friendId,
      );
  ...
  @HttpCode(HttpStatus.OK)
  async searchNonBlockedUser(
    @Param('username') username: string,
    @Req() req: Request,
  ) {
    return this.userService.searchSpecificUser(
      username,
      req.user,
    );
  }
  ...
  async searchUser(
    ...
    return this.userService.searchFriendUser(
      username,
      req.user,
    );
  }
  ...
  async getAvatar(
    ...
    const fs = require('fs');
    if (
      fs.existsSync(process.cwd() + filenamePath)
    ) {
  ...
  async checkBlockedFriend(
    ...
    return await this.userService.checkBlockedFriend(
      req.user['sub'],
      friendId,
    );
  }
  ...
  async myFriendFriends(
    ...
    return await this.userService.friendFriends(
      user.id,
      friendId,
    );
  }
