...
export class HistoryService {
  ...
  async getLatestGames(userId: number) {
    try {
      ...
      console.log(
        `Something wrong happend with database ${error}`,
      );
    }
  }

  async createResultGame(
    userId: number,
    body: historyDto,
  ) {
    try {
      ...
      if (
        newGameAdded.myScore >
        newGameAdded.oppScore
      ) {
        await this.game.updateInfoGame(
          newGameAdded.myUserId,
          true,
        );
        await this.game.updateInfoGame(
          newGameAdded.oppUserId,
          false,
        );
      } else {
        await this.game.updateInfoGame(
      ...
      console.log(
        `Something wrong happend with database ${err}`,
      );
    }
  }
}
