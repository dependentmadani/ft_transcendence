...
export class GameService {
  ...
  async getInfoGame(userId: number) {
    try {
      ...
      console.log(
        `Something wrong with database! !!!!!!! ${e}`,
      );
    }
  }

  async updateInfoGame(
    userId: number,
    win_state: boolean,
  ) {
    try {
      ...
      let win = game.wins,
        lose = game.loses,
        played = game.gamesPlayed;
      win_state == true ? win++ : lose++;
      played += 1;
      if (win_state == true) {
        const score = game.score + 15;
        const user =
  ...
  async leaderboard() {
    try {
      ...
      console.log(
        'something wrong with catching leaderboard content',
      );
    }
  }
  ...
  async playerPosition(username: string) {
    try {
      ...
      console.log(
        'something wrong with the leaderboard/username api',
      );
    }
  }
}
