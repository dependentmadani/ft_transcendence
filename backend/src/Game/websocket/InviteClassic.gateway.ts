...
console.log(
  `HELLO FROM SCRIPT CLASSIC___________INVITE`,
);
@WebSocketGateway({
...
export class InviteClassicSocketGateway {
  ...
  async handleyoucanstart(client, user) {
    ...
    if (!this.PosPlayers.has(this.client_id)) {
      if (this.count <= 2) {
        if (this.count <= 2) {
          if (this.client_id === this.prfl) {
            ...
            console.log(
              `Player ${this.client_id} joined room ${this.room}`,
            );
            ...
            if (key) {
              ...
              if (
                roomArray.length === 2 &&
                this.PosPlayers.has(roomArray[0])
              )
                this.count += 2;
              ...
              console.log(
                `Player ${this.client_id} joined room ${key}`,
              );
              client.join(key);
            }
          }
        ...
        //   client.join(this.room);
        // }
        // // if(this.PosPlayers.has(this.prfl))
        // // {
        //   let keys = this.findKeyByValue(this.rooms, this.prfl);
        //   // console.log(`${this.profileID2} ------111111-| ${this.PosPlayers.get(this.client_id)}| ---|||| ${ keys}`)
        //   if ((this.client_id == this.adver))
        //   {
        //     while (!this.rooms.has(keys))

        //     {
        //       console.log(`${this.profileID2} ------22222-| ${this.PosPlayers.get(this.client_id)}| ---|||| ${ keys}`)
        ...
        //     this.profileID2 = this.client_id;
        //     if(roomArray.length == 2 && this.PosPlayers.has(roomArray[0]))
        //           this.count+= 2;
      ...
      if (this.prev_room != this.room) {
        this.pl1[this.room] =
          await new paddle_left();
        this.pl2[this.room] =
          await new paddle_right();
        this.ball[this.room] =
          await new direct_ball();
        this.prev_room = this.room;
      }
      this.pl1[this.room].canvas_height =
        this.canvas_height;
      this.pl2[this.room].canvas_height =
        this.canvas_height;
      this.pl1[this.room].paddle_y =
        this.canvas_height / 2;
      this.pl2[this.room].paddle_y =
        this.canvas_height / 2;
      this.pl1[this.room].height_paddle =
        (this.canvas_height * 20) / 100;
      this.pl2[this.room].height_paddle =
        (this.canvas_height * 20) / 100;
      this.ball[this.room].cnv_w =
        this.canvas_width;
      this.ball[this.room].cnv_h =
        this.canvas_height;
      this.ball[this.room].ball_x =
        this.canvas_width / 2;
      this.ball[this.room].ball_y =
        this.canvas_height / 2;
    }
  }
  ...
  async startGameIfNeeded(room_num, client) {
    if (this.pass && this.count == 2) {
      this.count = 0;
      this.interval[room_num] = await setInterval(
        () => this.game(room_num, client),
        1000 / 60,
      );
    }
  }
  ...
  async game(room_num, client) {
    this.ball[room_num].p_left =
      this.pl1[room_num].paddle_y;
    this.ball[room_num].p_right =
      this.pl2[room_num].paddle_y;
    const gameState = {
      ...
      profileID2: {},
      ball: {
        x: this.ball[room_num].ball_x,
        y: this.ball[room_num].ball_y,
      },
      paddles: {
        left: this.pl1[room_num].paddle_y,
        right: this.pl2[room_num].paddle_y,
      },
      scores: {
        player1: this.ball[room_num].score_left,
        player2: this.ball[room_num].score_right,
      },
      sound: {
        sound_paddle:
          this.ball[room_num].sound_padd,
        sound_wall:
          this.ball[room_num].sound_wall,
      },
    };
    ...
    if (this.ball[room_num].reset_game_flag) {
      ...
      this.pl2[room_num].paddle_y =
        this.canvas_height / 2;
      this.ball[room_num].ball_x =
        this.canvas_width / 2;
      this.ball[room_num].ball_y =
        this.canvas_height / 2;
    ...
    gameState.scores.player1 =
      this.ball[room_num].score_l;
    gameState.scores.player2 =
      this.ball[room_num].score_r;
    ...
    if (
      this.ball[room_num].score_left >=
        this.ball[room_num].score_max ||
      this.ball[room_num].score_right >=
        this.ball[room_num].max_score
    ) {
      const roomSet = this.rooms.get(room_num);

      if (roomSet) {
        const roomArray = Array.from(roomSet);
        const firstValue = roomArray[0];
        const secondValue = roomArray[1];

        this.histor1.my_score =
          this.ball[room_num].score_left;
        this.histor1.opp_score =
          this.ball[room_num].score_right;
        this.histor1.opp_name =
          this.client_name.get(secondValue);
        this.historyService.createResultGame(
          firstValue,
          this.histor1,
        );
      ...
      const sett = this.rooms.get(room_num);
  ...
  handlePlayerDisconnect(client, data) {
    ...
    if (room) {
      if (this.PosPlayers.get(clientId) == 1)
        this.ball[room].score_r =
          this.ball[room].score_max;
      else
        this.ball[room].score_l =
          this.ball[room].score_max;
    }
  }
