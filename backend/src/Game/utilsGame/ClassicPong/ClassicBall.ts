export default class direct_ball {
  ...
  check_colision() {
    if (
      this.y <= 0 ||
      this.y > this.canvas_height - this.ball_size
    ) {
      this.direction_y *= -1;
      this.sound_wall++;
  ...
  ball_paddle_colision() {
    if (
      this.x < this.width_paddle &&
      this.y + this.ball_size >= this.p_left &&
      this.y <= this.p_left + this.height_paddle
    ) {
      ...
      this.x +
        (this.ball_size + this.width_paddle) >
        this.canvas_width &&
      this.y + this.ball_size >= this.p_right &&
      this.y <= this.p_right + this.height_paddle
    ) {
      this.x =
        this.canvas_width -
        this.width_paddle -
  ...
  set score_l(nb: number) {
    this.score_left = nb;
  }
  get score_r() {
    return this.score_right;
  }
  set score_r(nb: number) {
    this.score_right = nb;
  }
  get score_max() {
